...
def get_model_actor_image(input_dims, output_dims):
    ...
    model.add_loss(ppo_loss([], out_actions, oldpolicy_probs, advantages, rewards, values))
...
if image_based:
    env = football_env.create_environment(env_name='academy_empty_goal', representation='pixels', render=True, rewards='checkpoints,scoring')
else:
...
while not target_reached and iters < max_iters:
    ...
    x = {
        "input_1": np.asarray(states),
        "input_2": np.asarray(actions_probs),
        "input_3": np.asarray(advantages),
        "input_4": np.asarray(np.reshape(rewards, newshape=(-1, 1, 1))),
        "input_5": np.asarray(values[:-1])
    }

    actor_loss = model_actor.fit(
        x,
        [(np.reshape(actions_onehot, newshape=(-1, n_actions)))], verbose=True, shuffle=True, epochs=10,
        callbacks=[tensor_board])

    x_critic = {
        "input_7": np.asarray(states)
    }

    critic_loss = model_critic.fit(x_critic, [np.reshape(returns, newshape=(-1, 1))], shuffle=True, epochs=10,
